...
std::vector<SoundBank> IWeaponLoader::parseSoundBanksJson(const std::string& jsonText) {
    ...
    for (const auto& item : j) {
        BasicWeaponProfile profile;
        if (!BasicWeaponProfile::from_json(item, profile)) {
            continue;
        }
...
std::vector<SoundBank> IWeaponLoader::loadSoundBanks(const std::string& jsonFilePath) const {
